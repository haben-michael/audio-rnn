...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
      out, cache = rnn_forward(out_word,h0,Wx,Wh,b)
    ## (4-5 )
    ...
    grads['x'], dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh,cache)
    grads['W_embed'] = word_embedding_backward(grads['x'],cache_word)    
  ...
  def sample(self, features, max_length=30):
    ...
    words = self._start*np.ones((N,1),dtype=np.int16)
    for t in xrange(max_length):
      # import pdb;pdb.set_trace()  
      words,_ = word_embedding_forward(words,W_embed)
      words = words[:,0,:]
      hidden_state, _ = rnn_step_forward(words,hidden_state,Wx,Wh,b)
